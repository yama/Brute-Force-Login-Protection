...
class Htaccess {
    ...
    private function __addLine($line) {
        $lines = $this->__getLines(array('deny from ', 'ErrorDocument 403 '));
        $c = count($lines);
        while(100<$c && array_shift($lines)) {
            $c--;
        }
        $insertion = array_merge($this->__header, $lines, array($line), $this->__footer);
